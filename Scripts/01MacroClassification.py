...
caminho_arquivo = '06_Datasets.xlsx'
df_split_fair = pd.read_excel(caminho_arquivo)

df_clear = df_split_fair[df_split_fair['is_encrypted'] == 0].copy()
...
# ---------------------------------------------------------
# 4. Construção dos Lotes (Restauração da Lógica Original)
# ---------------------------------------------------------
tamanho_lote = 1 
lotes_de_variaveis = [colunas_para_classificar[i:i + tamanho_lote] for i in range(0, len(colunas_para_classificar), tamanho_lote)]

# ---------------------------------------------------------
# 5. Passagem 1: Classificação Inicial
# ---------------------------------------------------------
print(f"\nA processar Passagem 1 ({len(lotes_de_variaveis)} lotes criados)...")
...
# ---------------------------------------------------------
# 6. Preparação e Passagem 2: Auditoria
# ---------------------------------------------------------
print("\nA preparar e processar Passagem 2 (Reflexão)...")
mensagens_reflexao = []
lotes_aprovados_para_reflexao = []

for lote_atual, output in zip(lotes_de_variaveis, resultados_classificacao):
    ...
    try:
        ...
        lotes_aprovados_para_reflexao.append(lote_atual)
    except Exception as e:
        # Imita a lógica original: se falhar na conversão, o lote morre aqui e não atualiza o dicionário
        print(f"Erro no lote {lote_atual} (Passagem 1): {str(e)[:50]}")
...
# ---------------------------------------------------------
# 7. Atualização do Dicionário e Exportação
# ---------------------------------------------------------
print("\nA consolidar o dicionário final...")
dicionario_final = {}

for lote_atual, output in zip(lotes_aprovados_para_reflexao, resultados_reflexao):
    resp_texto = output.outputs[0].text
    try:
        json_final = extrair_json_dict(resp_texto)
        dicionario_final.update(json_final) # Atualiza todas as variáveis que estavam dentro deste lote
    except Exception as e:
        print(f"Erro no lote {lote_atual} (Passagem 2): {str(e)[:50]}")
...
caminho_saida = 'MacroTaxonomyClassification.csv'
